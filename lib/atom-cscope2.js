...
export default {
  ...
  activate(state) {
    ...
    this.menuView = new MenuView(
      {
        onCancel: this.returnToLastPane.bind(this),
        onConfirm: (query, keyword) => {
          this.returnToLastPane()
          this.searchCscope(query, keyword)
        }
      }
    );
    ...
    this.resultView = new SelectListView(
      {
        ...
        didCancelSelection: this.returnToLastPane.bind(this),
  ...
  toggle() {
    if (this.resultPanel.isVisible())
      ...
      const keyword = this.findSelection();
      if ((keyword == null))
        return
      this.searchCscope('1', keyword)
    }
  },

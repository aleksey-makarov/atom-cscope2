  ...
  cscope(query, keyword)
    .then(result => {
      if (result.length === 1) {
        ...
        setLastPane()
        resultView.update({ items: result, query: '' });
        if (!resultPanel.isVisible()) {
          resultPanel.show();
          resultView.focus();
        }
...
export default {
  ...
  activate(state) {
    ...
    resultView = new SelectListView(
      {
        ...
        filterKeyForItem: item =>
          (item.projectPath ? path.basename(item.projectPath) + " " : "") + item.fileName
      }
    );
    ...
    subscriptions.add(atom.commands.add('atom-workspace',
      {'atom-cscope2:goToDefinition': goToDefinition  }));
    subscriptions.add(atom.commands.add('atom-workspace',
      {'atom-cscope2:showMenu': showMenu }));
    subscriptions.add(atom.commands.add('atom-workspace',
      {'atom-cscope2:next': history.next }));
    subscriptions.add(atom.commands.add('atom-workspace',
      {'atom-cscope2:prev': history.prev }));
  },
